...
async def test_user_flow_requests_device_code_and_creates_entry(hass, monkeypatch) -> None:
    ...
    assert result["data"][CONF_SELECTED_VINS] == ["TESTVIN1234567890"]
    assert result["data"][CONF_CONTAINER_ID] == "C123"
    assert result["data"][CONF_CONTAINER_NAME] == "ha_ev_current"


async def test_user_flow_supports_multi_vehicle_selection(hass, monkeypatch) -> None:
    async def mock_request_device_code(self):
        return (
            BMWDeviceCodePayload(
                user_code="ABCD1234",
    ...
    async def mock_get_vehicle_mappings(self):
        return [
            {"vin": "TESTVIN1234567890", "mappingType": "PRIMARY"},
            {"vin": "TESTVIN0987654321", "mappingType": "PRIMARY"},
        ]
    ...
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        {CONF_SELECTED_VINS: ["TESTVIN1234567890", "TESTVIN0987654321"]},
    )
    assert result["type"] == "create_entry"
    assert result["title"] == "BMW CarData (2 vehicles)"
